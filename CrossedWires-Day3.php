...
function pathRangeCheck($first, $next, $find) {
    if ($first < $next) {
...
function findCrossedWires($wire1, $wire2) {
    $crossed = array();
    for($i = 0; $i < (count($wire1)-1); $i++) {
        $cur_move = $wire1[$i];
        $next_move = $wire1[$i+1];
        
        for($t = 0; $t < (count($wire2)-1); $t++) {
            $cur_path = $wire2[$t];
            $next_path = $wire2[$t+1];
            
            if ($cur_move["x"] != $next_move["x"]) {
                if (pathRangeCheck($cur_move["x"], $next_move["x"], $cur_path["x"])) {
                    if (pathRangeCheck($cur_path["y"], $next_path["y"], $cur_move["y"])) {
                        $cross_index = count($crossed) + 1;
                        print("</br>".$cur_path["x"].":".$cur_path["y"]);
                        print("</br>".$next_path["x"].":".$next_path["y"]);
                        print("</br>".$cur_move["x"].":".$cur_move["y"]);
                        print("</br>".$next_move["x"].":".$next_move["y"]."<br/>");
                        $crossed[$cross_index] = abs($cur_path["x"]) + abs($cur_move["y"]);
                    }
                }
            } else {
                if (pathRangeCheck($cur_move["y"], $next_move["y"], $cur_path["y"])) {
                    if (pathRangeCheck($cur_path["x"], $next_path["x"], $cur_move["x"])) {
                        $cross_index = count($crossed) + 1;
                        print("</br>".$cur_path["x"].":".$cur_path["y"]);
                        print("</br>".$next_path["x"].":".$next_path["y"]);
                        print("</br>".$cur_move["x"].":".$cur_move["y"]);
                        print("</br>".$next_move["x"].":".$next_move["y"]."<br/>");
                        $crossed[$cross_index] = abs($cur_move["x"]) + abs($cur_path["y"]);
                    }
                }
            }
        }
...
$crossed_wires = findCrossedWires(findPath($base_pos, $path1), findPath($base_pos, $path2));
